...
preset_templates = {
        ...
        "c": "我在百科全书中了解到<subj>是<obj>的<mask>",
        "d": "我在百科全书中了解到<subj>的<mask>包括<obj>",
        "e": "通过学习，我发现了<subj>和<obj>之间的关系：<mask>",
        "f": "<subj>是<obj>的<mask>，这表明它们之间存在着关系"
    }
...
class ChiRelPrompt:
    ...
    def __init__(self,
        ...
        # check if the language model is ChiRelPrompt trained or not.
        self.template = None
        self.custom_template = None
        if 'chirelprompt_config' in model_config.to_dict().keys():
            logging.info('loading finetuned ChiRelPrompt model')
            self.mode = model_config.chirelprompt_config['mode']
            ...
            if template_type in preset_templates:
                ...
                model_config.update({'chirelprompt_config': {'mode': mode, 'template': self.template}})
        try:
    ...
    def get_embedding(self, x: List, batch_size: int = None, num_worker: int = 1):
        """ Get embedding from ChiRelPrompt (no gradient).
